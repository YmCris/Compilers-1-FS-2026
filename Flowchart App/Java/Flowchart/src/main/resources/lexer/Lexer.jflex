...
%{
    ...
    private ArrayList<ArrayList<Symbol>> lines = new ArrayList<>();
    

    // SPECIFIC METHODS --------------------------------------------------------
    private Symbol register(Symbol symbol) {
        int lineIndex = symbol.left - 1;

        while (lines.size() <= lineIndex) {
            lines.add(new ArrayList<>());
        }

        lines.get(lineIndex).add(symbol);
        return symbol;
    }
    ...
    public ArrayList<ArrayList<Symbol>> getLines() {
        return lines;
    }
